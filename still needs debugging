...
def Ising_simulation(n, steps, J, T, r, ifcorr, ifreset):
    ...
    if ifcorr == True:
        ...
        return lattice, ave_e, specific_heat, M
...
def E_specific_heat_M_data(j, T_range, N_T, method):
    ...
    if method == 1: #reset 
        ...
        return
    for t in T_data:
        _, energy, specific_heat, magnetization = Ising_simulation(n=100, steps=100000, J=j, T=t, r=1, ifcorr=False, ifreset=IF)
        E_data.append(energy)
        SH_data.append(specific_heat)
        M_data.append(magnetization)
        
    return T_data, E_data, SH_data, M_data
...
temperature_data1,energy_data1, _,magnetization_data1 = E_specific_heat_M_data(j=1, T_range=[0.1,5], N_T=100, method=1)
temperature_data2,energy_data2,specific_heat_data2,magnetization_data2 = E_specific_heat_M_data(j=1, T_range=[0.1,5], N_T=100, method=2)
temperature_data3,energy_data3,specific_heat_data3,magnetization_data3 = E_specific_heat_M_data(j=1, T_range=[0.1,5], N_T=100, method=3)
